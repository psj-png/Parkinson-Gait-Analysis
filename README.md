...
### 3. 조기 진단을 위한 이진 분류 모델 (Binary Classification)
* **목적**: 상세 병명 진단이 아닌, '조기 발견'과 '병원 방문 권고'를 최우선 목표로 설정.
* **분류 체계 변경**: 
  - 기존 3단계 분류에서 **[정상(Normal) vs 의심(Suspected)]** 2단계로 단순화.
  - 모호한 데이터(Ambiguous)를 '의심'군으로 통합하여 진단 누락(False Negative)을 방지함.
* **High-Recall 전략**: 
  - 실제 환자를 정상으로 오진하는 위험을 최소화하기 위해 모델의 민감도를 극대화.
  - "보행 이상 징후가 조금이라도 발견되면 즉시 전문의 진료를 권고"하는 스크리닝 도구로서의 역할 강조.
