...
파킨슨병을 비롯한 뇌 질환은 조기 발견이 치료 결과를 크게 좌우한다. 파킨슨병의 대표적인 초기 증상 중 하나가 보행 이상이지만, 환자 스스로 이를 인지하고 병원을 찾기까지 평균 수년이 걸린다. 이 기간 동안 질환은 조용히 진행되고, 치료 가능한 골든타임은 점점 좁아진다.

이 프로젝트의 출발점은 **"스마트폰 카메라만으로 보행 이상을 조기에 감지해, 환자가 더 빨리 병원을 찾도록 돕는 것"** 이었다. 별도의 웨어러블 기기나 전문 장비 없이, 일상에서 쉽게 촬영한 영상 하나로 이상 징후를 포착할 수 있다면 접근성과 파급력 모두를 잡을 수 있다고 판단했다.

초기에는 파킨슨 확진 환자의 보행 영상을 직접 학습 데이터로 활용하려 했으나, 공개된 파킨슨 보행 영상 데이터셋이 전 세계적으로 극히 부족해 현실적으로 불가능했다. 이에 **정상 보행 vs 이상 보행 이진 분류**로 목표를 전환했다. 파킨슨 특이적 진단 대신, 보행 패턴의 이상 여부를 감지해 조기 검진을 권고하는 방향으로 설계함으로써 데이터 제약을 우회하면서도 실질적인 임상적 가치를 유지했다.

**동작 방식**

보행 영상이 입력되면 인물을 추적·크롭하고, Optical Flow 기반으로 프레임 간 움직임 패턴을 추출한다. 추출된 시퀀스를 분류 모델에 통과시켜 정상/이상 여부를 판별하며, 이상 보행이 감지될 경우 사용자에게 조기 검진을 권고한다. 별도 앱 설치 없이 웹 브라우저에서 바로 사용할 수 있도록 Flask 기반 웹 인터페이스를 제공한다.
