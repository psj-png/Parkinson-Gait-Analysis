...
# 3-1. 벌점(가중치) 기준을 먼저 정의합니다. (반드시 모델 생성 코드보다 위에 있어야 함)
custom_weights = {
    '01_Normal': 1,      # 정상: 기준 벌점 1
    '03_Ambiguous': 1,   # 모호함: 기준 벌점 1
    '02_Parkinson': 10    # 파킨슨: 놓치면 벌점 10배 (더 엄격하게 판정)
}

# 3-2. 정의한 가중치를 모델에 적용합니다.
model = RandomForestClassifier(
    n_estimators=100,
    max_depth=10,
    class_weight=custom_weights,  # 이제 컴퓨터가 custom_weights를 인식합니다!
    random_state=None             # 매번 새로운 결과 확인을 위해 None 설정
)
